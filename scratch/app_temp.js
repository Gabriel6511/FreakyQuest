...
function document.querySelector(query) {
  if (!query) return null;
  if (!DOM_CACHE[query]) {
    DOM_CACHE[query] = document.querySelector(query);
  }
  return DOM_CACHE[query];
}

// 4. GLOBAL CONFIGURATION FOR GAMEPLAY METRICS (ELIMINATES MAGIC NUMBERS)
const FREAKY_CONFIG = {
  DEFAULT_REST_TIME: 90,
  BASE_XP_WATER: 15,
  VIG_MULTIPLIER_SCALE: 0.02,
  AGI_MULTIPLIER_SCALE: 0.01,
  FOC_MULTIPLIER_SCALE: 0.01,
  XP_BUFF_ANABOLIC: 1.10, // +10% XP
  DEFAULT_WEIGHT: 70,
  DEFAULT_HEIGHT: 170
};

// 5. UNIFIED GEAR BONUS HELPER (DRY PRINCIPLE)
  ...
  if (state.equippedItems) {
    Object.values(state.equippedItems).forEach(itemId => {
      if (itemId) {
        ...
        if (item && item.stats && item.stats[stat] !== undefined) {
...
function getEffectiveAttributes() {
  ...
  for (const stat in eff) {
    eff[stat] += getGearBonus(stat);
  }
  return eff;
}
...
function updateWaterTargetFromWeight() {
  const weight = parseFloat(state.charWeight) || FREAKY_CONFIG.DEFAULT_WEIGHT;
  state.waterTarget = Math.max(2, Math.min(6, Math.round(weight * 0.035)));
}
...
function recalculateMacrosTargets() {
  const weight = parseFloat(state.charWeight) || FREAKY_CONFIG.DEFAULT_WEIGHT;
  const targetW = parseFloat(state.targetWeight) || weight;
  const height = parseFloat(state.charHeight) || FREAKY_CONFIG.DEFAULT_HEIGHT;
  ...
  if (state.charGoal === 'engordar' || state.charGoal === 'estetico') {
    refWeight = Math.max(weight, targetW);
  } else if (state.charGoal === 'emagrecer') {
    refWeight = Math.min(weight, targetW);
  }
...
function applyMentorVisualTheme(mentor) {
  ...
  if (mentor.id === 'bebezinho') {
    if (!particlesContainer) {
      ...
      for (let i = 0; i < 15; i++) {
        ...
        particleEl.className = 'bebezinho-sparkle';
        particleEl.style.left = `${Math.random() * 100}%`;
        particleEl.style.top = `${Math.random() * 100}%`;
        particleEl.style.animationDelay = `${Math.random() * 3}s`;
        particleEl.style.animationDuration = `${2 + Math.random() * 4}s`;
  ...
  if (mentor.isCustom) {
    ...
    const hue = mentor.hue;
    styleTag.innerHTML = `
      body.theme-custom-${mentor.id} {
        --primary-hue: ${hue};
        --primary-sat: 90%;
        --primary-light: 50%;
        --accent-hue: ${(hue + 150) % 360};
        --accent-sat: 95%;
        --accent-light: 50%;
      }
    `;
...
// 11. UI RENDERING SYSTEM SUB-RENDERERS (SRP & PERFORMANCE OPTIMIZATION)
// ==========================================================================

function updateUIPlayerHeader(activeMentorData, classLabels, xpPercent, mentorAvatarSrc, userAvatarSrc) {
  ...
  // Dynamic header showcase trophies sync (bypassing cache since parent innerHTML is replaced)
  ...
  if (headerTrophiesContainer) {
    ...
    if (state.showcaseTrophies && state.showcaseTrophies.length > 0) {
      state.showcaseTrophies.forEach(slotId => {
        const trophy = TROPHIES.find(item => item.id === slotId);
        if (trophy) {
          const badgeSpan = document.createElement('span');
          badgeSpan.innerText = trophy.icon;
          badgeSpan.title = trophy.name;
  ...
  if (lvlBadge) lvlBadge.innerText = `Lvl ${state.level}`;
  ...
  if (classNameEl) {
    ...
    classNameEl.innerText = classLabels[uClass] || uClass;
  }

  const currentRank = getSubclassRank(userProfile.class || state.charClass || 'bodybuilder', state.level);
  const playerRankEl = document.getElementById('player-rank');
  if (playerRankEl) playerRankEl.innerText = currentRank;

  const xpTextEl = document.getElementById('xp-text');
  if (xpTextEl) xpTextEl.innerText = `${state.xp} / ${state.xpNeeded} XP`;
  ...
  if (headerAv) headerAv.src = userAvatarSrc;

  const statusAv = document.getElementById('status-avatar');
  if (statusAv) statusAv.src = userAvatarSrc;

  // Apply aura styles
  const headerAvatarWrapper = headerAv ? headerAv.parentElement : null;
  const statusAvatarWrapper = statusAv ? statusAv.parentElement : null;
  ...
  if (heroSub) {
    const uClass = userProfile.class || state.charClass || 'bodybuilder';
    const classLabel = classLabels[uClass] || uClass;
...
function updateUIIMCFicha(goalsLabels) {
  ...
  const weight = parseFloat(userProfile.currentWeight) || parseFloat(state.charWeight) || FREAKY_CONFIG.DEFAULT_WEIGHT;
  const height = parseFloat(userProfile.height) || parseFloat(state.charHeight) || FREAKY_CONFIG.DEFAULT_HEIGHT;
  
  const evalWeight = document.getElementById('eval-weight');
  if (evalWeight) evalWeight.innerText = `${weight} kg`;
  
  const evalHeight = document.getElementById('eval-height');
  if (evalHeight) evalHeight.innerText = `${height} cm`;
  
  const hMeters = height / 100;
  const imc = weight / (hMeters * hMeters);
  const evalImc = document.getElementById('eval-imc');
  if (evalImc) evalImc.innerText = imc.toFixed(1);
  ...
  const evalImcClass = document.getElementById('eval-imc-class');
  if (evalImcClass) evalImcClass.innerText = imcCat;
  
  const evalRpgTip = document.getElementById('eval-rpg-tip');
  if (evalRpgTip) evalRpgTip.innerHTML = imcRpgTip;
  
  const evalGoalBadge = document.getElementById('eval-goal-badge');
  if (evalGoalBadge) {
    evalGoalBadge.innerText = goalsLabels[userProfile.mainObjective || state.charGoal] || state.charGoal || '—';
  }
...
function updateUIWaterSection() {
  const waterTargetInput = document.getElementById('water-target-input');
  if (waterTargetInput) waterTargetInput.value = state.waterTarget;
  ...
  const currentLitresVal = state.waterDrank || state.waterIntake || 0;
  if (waterText) waterText.innerText = `${currentLitresVal} / ${state.waterTarget} L`;
  
  // Update water cylinder fill level
  const cylinderFill = document.getElementById('water-cylinder-fill');
  if (cylinderFill) {
    const waterPct = Math.min(100, Math.round((currentLitresVal / state.waterTarget) * 100));
  ...
  if (extraWaterText) {
    if (currentLitresVal > state.waterTarget) {
      const extraL = currentLitresVal - state.waterTarget;
...
function updateUIDietSection(totalKcal, totalProt, totalFiber, totalCarbs, goalText) {
  ...
  const textKcal = document.getElementById('text-kcal');
  if (textKcal) textKcal.innerText = `${totalKcal}/${state.kcalTarget}`;
  const textProt = document.getElementById('text-prot');
  if (textProt) textProt.innerText = `${totalProt}g/${state.protTarget}g`;
  const textFiber = document.getElementById('text-fiber');
  if (textFiber) textFiber.innerText = `${totalFiber}g/${state.fiberTarget}g`;
  ...
  if (barKcal) barKcal.style.width = `${kcalBarPct}%`;
  const barProt = document.getElementById('bar-prot');
  if (barProt) barProt.style.width = `${protBarPct}%`;
  const barFiber = document.getElementById('bar-fiber');
  if (barFiber) barFiber.style.width = `${fiberBarPct}%`;

  const dockPct = Math.min(100, Math.round((totalProt / state.protTarget) * 100));
  const proteinDockPercent = document.getElementById('protein-dock-percent');
  if (proteinDockPercent) proteinDockPercent.innerText = `Proteínas: ${dockPct}% da meta`;
  ...
  // ==========================================
  const dietKcalSummary = document.getElementById('diet-kcal-summary');
  if (dietKcalSummary) dietKcalSummary.innerText = `${totalKcal} / ${state.kcalTarget} kcal`;
  const dietProtSummary = document.getElementById('diet-prot-summary');
  if (dietProtSummary) dietProtSummary.innerText = `${totalProt} / ${state.protTarget} g`;
  const dietFiberSummary = document.getElementById('diet-fiber-summary');
  if (dietFiberSummary) dietFiberSummary.innerText = `${totalFiber} / ${state.fiberTarget} g`;
  
  const kcalPercent = Math.min(100, Math.floor((totalKcal / state.kcalTarget) * 100));
  const dietKcalPercent = document.getElementById('diet-kcal-percent');
  if (dietKcalPercent) dietKcalPercent.innerText = `${kcalPercent}%`;

  // Anel KCAL — r=44, circunferência = 276.46
  const kcalCirc = 276.46;
  const dietKcalRingFill = document.getElementById('diet-kcal-ring-fill');
  if (dietKcalRingFill) dietKcalRingFill.style.strokeDashoffset = kcalCirc - (kcalPercent / 100) * kcalCirc;
  ...
  const dietCalorieStatus = document.getElementById('diet-calorie-status');
  if (dietCalorieStatus) dietCalorieStatus.innerText = goalText[state.charGoal] || state.charGoal || '—';
...
function updateUIWorkoutSection() {
...
function updateUIStatusTab(currentRank, classLabels) {
  const statsTotalWorkouts = document.getElementById('stats-total-workouts');
  if (statsTotalWorkouts) statsTotalWorkouts.innerText = state.workoutsCompleted;
  
  const statsLevelNum = document.getElementById('stats-level-num');
  if (statsLevelNum) statsLevelNum.innerText = state.level;
  
  const statusSubclass = document.getElementById('status-subclass');
  if (statusSubclass) statusSubclass.innerText = currentRank;
  
  const statusClass = document.getElementById('status-class');
  if (statusClass) statusClass.innerText = classLabels[state.charClass];
  ...
  const statsRankTier = document.getElementById('stats-rank-tier');
  if (statsRankTier) statsRankTier.innerText = rankChar;
  ...
  if (state.attrPoints > 0) {
    if (attrBox) attrBox.classList.remove('hidden');
    if (attrValText) attrValText.innerText = state.attrPoints;
    addBtnList.forEach(btn => btn.classList.remove('hidden'));
  } else {
    if (attrBox) attrBox.classList.add('hidden');
    addBtnList.forEach(btn => btn.classList.add('hidden'));
  }
}

function updateUIEquipmentAndTrophies() {
...
// Main updateUI Orchestrator
function updateUI() {
  checkDailyReset();
  recalculateMacrosTargets();

  // 1. Gather Shared UI Constants
  const classLabels = {
  ...
  const mentorAvatarSrc = getMentorAvatarSrc(activeMentorData);

  // Calculate Eaten today sums
  ...
  // 2. Call Specialized Sub-Renderers
  updateUIPlayerHeader(activeMentorData, classLabels, xpPercent, mentorAvatarSrc, userAvatarSrc);
  updateUIIMCFicha(goalsLabels);
  updateUIWaterSection();
  updateUIDietSection(totalKcal, totalProt, totalFiber, totalCarbs, goalText);
  updateUIWorkoutSection();
  updateUIStatusTab(currentRank, classLabels);
  updateUIEquipmentAndTrophies();

  // 3. Tab: Mentores rendering (outside status tab)
  renderMentorsList();

  // 4. Update Eternal Flame
  const flameCountEl = document.getElementById('eternal-flame-count');
  if (flameCountEl) {
    const clicks = state.eternalFlameClicks || 0;
    flameCountEl.innerText = `${clicks.toLocaleString('pt-BR')} toques de respeito`;
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (slotId) {
      ...
      if (trophy) {
        ...
        slotEl.innerHTML = `
          <span class="showcase-icon">${trophy.icon}</span>
          <span class="showcase-name">${trophy.name}</span>
        `;
        slotEl.title = `Clique para remover "${trophy.name}" da vitrine`;
...
function renderEvolutionChart() {
  const todayStr = '2026-06-24'; // June 24th, 2026 (matching system date)
  
  // Make sure dailyHistory is initialized
  initializeDailyHistory();
  
  // Sync today's data in history
  ...
  // 1. RENDER FITNESS RINGS (Apple style)
  const ringsSvg = document.getElementById('fitness-rings-svg');
  if (ringsSvg) {
    ringsSvg.innerHTML = '';
    
    // Add filters/defs for rings glow
    const defsRings = document.createElementNS('http://www.w3.org/2000/svg', 'defs');
    defsRings.innerHTML = `
      <filter id="ring-glow-red" x="-20%" y="-20%" width="140%" height="140%">
      ...
      <filter id="ring-glow-green" x="-20%" y="-20%" width="140%" height="140%">
        <feGaussianBlur stdDeviation="2.5" result="blur" />
        <feMerge>
          <feMergeNode in="blur" />
          <feMergeNode in="SourceGraphic" />
      ...
      <filter id="ring-glow-blue" x="-20%" y="-20%" width="140%" height="140%">
        <feGaussianBlur stdDeviation="2.5" result="blur" />
        <feMerge>
          <feMergeNode in="blur" />
          <feMergeNode in="SourceGraphic" />
    ...
    `;
    ringsSvg.appendChild(defsRings);

    let kcalTarget, kcalCurrent, workoutTarget, workoutCurrent, waterTarget, waterCurrent;

    if (selectedHistoryDate) {
      const entry = state.dailyHistory[selectedHistoryDate] || {
        kcal: 0, kcalTarget: 2500, water: 0, waterTarget: 3.0, workouts: 0, workoutTarget: 4
      };
      kcalTarget = entry.kcalTarget;
      kcalCurrent = entry.kcal;
      workoutTarget = entry.workoutTarget;
      workoutCurrent = entry.workouts;
      waterTarget = entry.waterTarget;
      waterCurrent = entry.water;
      ...
      if (fitTitle) {
        const dayPart = selectedHistoryDate.split('-')[2];
        fitTitle.innerText = `❤️ Painel de Fitness (${parseInt(dayPart)} de Junho)`;
      }
    } else {
      kcalTarget = state.kcalTarget || 2500;
      kcalCurrent = state.dailyMacros ? (state.dailyMacros.kcal || 0) : 0;
      workoutTarget = state.weeklyTrainGoal || 4;
      workoutCurrent = state.workoutsThisWeek || 0;
      waterTarget = state.waterTarget || 3.0;
      waterCurrent = state.waterDrank || 0;
      ...
      const fitTitle = document.getElementById('fitness-title');
      if (fitTitle) {
        fitTitle.innerText = `❤️ Painel de Fitness & Evolução`;
      }
    }

    const kcalProg = Math.min(1.0, Math.max(0.0, kcalCurrent / kcalTarget));
    const workoutProg = Math.min(1.0, Math.max(0.0, workoutCurrent / workoutTarget));
    const waterProg = Math.min(1.0, Math.max(0.0, waterCurrent / waterTarget));

    // Radii: Red=45, Green=33, Blue=21
    // Circumferences: Red=282.7, Green=207.3, Blue=131.9
    const rings = [
      { r: 45, color: '#ff0055', prog: kcalProg, circ: 282.7, glow: 'ring-glow-red', bg: 'rgba(255, 0, 85, 0.12)' },
      { r: 33, color: '#00ff66', prog: workoutProg, circ: 207.3, glow: 'ring-glow-green', bg: 'rgba(0, 255, 102, 0.12)' },
      { r: 21, color: '#00f2fe', prog: waterProg, circ: 131.9, glow: 'ring-glow-blue', bg: 'rgba(0, 242, 254, 0.12)' }
    ];
    ...
    rings.forEach(ring => {
      ...
      bgCircle.setAttribute('r', ring.r.toString());
      bgCircle.setAttribute('stroke', ring.bg);
      bgCircle.setAttribute('stroke-width', '10');
      bgCircle.setAttribute('fill', 'none');
      ringsSvg.appendChild(bgCircle);

      // Progress ring
      const fgCircle = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
      fgCircle.setAttribute('cx', '60');
      fgCircle.setAttribute('cy', '60');
      fgCircle.setAttribute('r', ring.r.toString());
      fgCircle.setAttribute('stroke', ring.color);
      fgCircle.setAttribute('stroke-width', '10');
      fgCircle.setAttribute('fill', 'none');
      fgCircle.setAttribute('stroke-linecap', 'round');
      fgCircle.setAttribute('transform', 'rotate(-90 60 60)');
      fgCircle.setAttribute('stroke-dasharray', ring.circ.toString());
      
      const offset = ring.circ * (1 - ring.prog);
      fgCircle.setAttribute('stroke-dashoffset', offset.toString());
      fgCircle.setAttribute('filter', `url(#${ring.glow})`);
      
      ringsSvg.appendChild(fgCircle);
    });

    // Update detailed labels
    const valKcal = document.getElementById('ring-val-kcal');
    if (valKcal) valKcal.innerText = `${Math.round(kcalCurrent)} / ${kcalTarget} kcal`;

    const valWorkouts = document.getElementById('ring-val-workouts');
    if (valWorkouts) valWorkouts.innerText = `${workoutCurrent} / ${workoutTarget} treinos`;

    const valWater = document.getElementById('ring-val-water');
    if (valWater) valWater.innerText = `${(waterCurrent).toFixed(1)} / ${(waterTarget).toFixed(1)} L`;

    // Update progress bars widths
    const barKcal = document.getElementById('ring-bar-kcal');
    if (barKcal) barKcal.style.width = `${kcalProg * 100}%`;

    const barWorkouts = document.getElementById('ring-bar-workouts');
    if (barWorkouts) barWorkouts.style.width = `${workoutProg * 100}%`;

    const barWater = document.getElementById('ring-bar-water');
    if (barWater) barWater.style.width = `${waterProg * 100}%`;
  }
  ...
  defs.innerHTML = `
    ...
    <linearGradient id="area-grad-strength" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0%" stop-color="#ff5e00" stop-opacity="0.15"/>
      <stop offset="100%" stop-color="#ff5e00" stop-opacity="0.0"/>
    </linearGradient>
    <linearGradient id="area-grad-weight" x1="0" y1="0" x2="0" y2="1">
  ...
  // Draw Area Gradients (draw before lines so they sit underneath)
  function drawArea(points, gradId) {
  ...
  drawArea(strengthPoints, 'area-grad-strength');
  drawArea(weightPoints, 'area-grad-weight');
  ...
  // Draw Pulse and Interactive Nodes
  function drawDots(points, color, typeLabel, unit) {
    points.forEach((p, idx) => {
      // Glowing radar effect
      const glowCircle = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
      glowCircle.setAttribute('cx', p.x.toString());
      glowCircle.setAttribute('cy', p.y.toString());
      glowCircle.setAttribute('r', '5');
      glowCircle.setAttribute('fill', color);
      glowCircle.setAttribute('class', 'chart-node-glow');
  ...
  // Interactive Hover Guide & Tooltip Engine
  const tooltip = document.getElementById('chart-tooltip');
  if (tooltip) {
    const focusLine = document.createElementNS('http://www.w3.org/2000/svg', 'line');
    focusLine.setAttribute('class', 'chart-focus-line');
    focusLine.setAttribute('x1', '0');
    focusLine.setAttribute('y1', padding.toString());
    focusLine.setAttribute('x2', '0');
    focusLine.setAttribute('y2', (height - padding).toString());
    ...
    function updateFocus(e) {
      ...
      // Scale to internal SVG viewBox width (400)
      ...
      const pS = strengthPoints[closestIdx];
      const pW = weightPoints[closestIdx];

      focusLine.setAttribute('x1', pS.x.toString());
      focusLine.setAttribute('x2', pS.x.toString());
      focusLine.style.display = 'block';

      tooltip.style.display = 'block';
      tooltip.innerHTML = `
        <div style="font-weight: 800; color: var(--color-primary); margin-bottom: 4px; font-size: 0.68rem; display: flex; align-items: center; gap: 4px;">📅 Registro ${closestIdx + 1}</div>
        <div style="display: flex; justify-content: space-between; gap: 12px; margin-bottom: 2px;">
          <span style="color: #ff9f1c; font-weight: 700;">✊ Força:</span>
          <span style="color: #ff9f1c; font-weight: bold;">${Math.round(pS.val * 10) / 10} XP</span>
        </div>
        <div style="display: flex; justify-content: space-between; gap: 12px;">
          <span style="color: #00f2fe; font-weight: 700;">⚖️ Peso:</span>
          <span style="color: #00f2fe; font-weight: bold;">${Math.round(pW.val * 10) / 10} kg</span>
        </div>
      `;

      // Snapped percentage positioning
      const midY = (pS.y + pW.y) / 2;
      tooltip.style.left = ((pS.x / width) * 100) + '%';
      tooltip.style.top = ((midY / height) * 100) + '%';
    }

    function showLatestTooltip() {
      if (pointsCount === 0) return;
      const lastIdx = pointsCount - 1;
      const pS = strengthPoints[lastIdx];
      const pW = weightPoints[lastIdx];
      ...
      tooltip.style.display = 'block';
      tooltip.innerHTML = `
        <div style="font-weight: 800; color: var(--color-accent); margin-bottom: 4px; font-size: 0.68rem; display: flex; align-items: center; gap: 4px;">⚡ Recente</div>
        <div style="display: flex; justify-content: space-between; gap: 12px; margin-bottom: 2px;">
          <span style="color: #ff9f1c; font-weight: 700;">✊ Força:</span>
          <span style="color: #ff9f1c; font-weight: bold;">${Math.round(pS.val * 10) / 10} XP</span>
        </div>
    ...
    // Initialize with latest tooltip
    showLatestTooltip();

    // Event listeners
    svg.addEventListener('mousemove', updateFocus);
    svg.addEventListener('touchmove', updateFocus, { passive: true });
    svg.addEventListener('mouseleave', showLatestTooltip);
    svg.addEventListener('touchend', showLatestTooltip);
  }

  // Draw the historical calendar grid
...
function initializeDailyHistory() {
  ...
  // Check if we need to pre-populate (if history has less than 5 keys)
  if (keys.length < 5) {
    function seededValue(seedStr, min, max) {
      ...
      const pct = Math.abs(hash % 100) / 100;
    ...
    for (let day = 1; day <= 23; day++) {
      ...
      // Weekly workouts cumulative count simulation
      const dayOfWeek = (day - 1) % 7;
      let workouts = 0;
      if (dayOfWeek === 0) workouts = 1;
      else if (dayOfWeek === 1 || dayOfWeek === 2) workouts = 2;
      else if (dayOfWeek === 3) workouts = 3;
      else workouts = 4;

      const kcal = Math.floor(seededValue(dateStr, 1600, 2600));
      const kcalTarget = state.kcalTarget || 2500;
      const water = parseFloat(seededValue(dateStr, 1.2, 3.6).toFixed(1));
      const waterTarget = state.waterTarget || 3.0;
...
function renderCalendarHistory() {
  ...
  const todayDayNum = 24;

  // Render days 1 to 30 for June 2026
  for (let day = 1; day <= 30; day++) {
    const dateStr = `2026-06-${day.toString().padStart(2, '0')}`;
    const entry = state.dailyHistory ? state.dailyHistory[dateStr] : null;
    ...
    if (selectedHistoryDate === dateStr) {
      ...
      // Highlight today if nothing else is selected
      cell.classList.add('today-highlight');
    } else if (day === todayDayNum) {
      // Highlight today if something else is selected too
      cell.classList.add('today-highlight');
    }
    ...
    if (day <= todayDayNum && entry) {
      ...
      // Mini rings configs: Outer (r=9, circ=56.5), Middle (r=6.5, circ=40.8), Inner (r=4, circ=25.1)
      const miniRings = [
