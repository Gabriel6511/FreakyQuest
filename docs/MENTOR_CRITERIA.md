...
7. **Item de equipamento (opcional, mas recomendado)** — dos 8 mentores atuais, só 5 têm um item real em `EQUIPMENT_DATABASE` (Faixa do Rock Lee, Aura do Goku, Aura do Broly, Capa do Saitama, Cinturão do Arnold); Bebezinho, Ramon Dino e Nick Walker não têm. Não é obrigatório, mas um item visual reforça a identidade do mentor na aba Itens — considerar sempre que o `tier1`/`tier2` do mentor já for do tipo `css_class` (ou seja, já é "um acessório visual" por natureza).
