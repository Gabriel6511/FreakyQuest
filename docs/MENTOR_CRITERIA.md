...
## 8. Atualização 2026-08-13 — 13º mentor: Aoi Todo (código pronto, imagem pendente)

- **Aoi Todo** (`todo`, universo já existente `Jujutsu Kaisen` — sem chip novo necessário), `effort`, FOR/RES (combinação diferente do Sukuna, que é FOR/AGI, pra manter builds distintas no mesmo universo), tema violeta "Boogie Woogie" (`#7209b7`).
- Falas fiéis usam ganchos reais do personagem: a técnica Boogie Woogie (bater palmas pra trocar de posição com quem tocou), a frase-filosofia real dele ("não odeio quem é fraco, odeio quem é fraco e não tenta melhorar"), o ritual de amizade "qual é o seu tipo?" e o callback "não é estranho, meu melhor amigo pensa igual" (referência à amizade com o Itadori).
- 3 itens, todos concretos/canônicos: **Palmas do Boogie Woogie** (Nv9·Punhos, a própria técnica), **Emblema de Kyoto Jujutsu High** (Nv19·Insígnia, a escola onde ele estuda), **Pôster da Takada Reika** (Nv30·Braços, a idol favorita dele dentro do universo de JJK — base real do "qual é seu tipo").
- **Status: todo o código já está em `app.js`/`styles.css` (mentor, progressão de 30 níveis, 3 itens, 5 falas fiéis, tema/aura), passou em `validate.py`/`check_duplicate_ids.py` e foi testado no console do navegador (mentor carrega, `UNIVERSE_ORDER` já cobre Jujutsu Kaisen, 23 recompensas geradas, 3 itens encontrados, 5 falas presentes).** Falta só: `todo.webp` (arte do mentor) + os 3 ícones dos itens (prompts em `docs/PROMPTS_ITENS_MENTORES.md`, seção "Aoi Todo") — sem isso, não foi commitado/enviado pro Vercel pra não subir avatar quebrado em produção. `filterCSS` está no preset "Anime" padrão como ponto de partida, precisa recalibrar lado a lado assim que a arte real chegar (mesma lição do Sukuna/Anya).
